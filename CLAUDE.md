...
- **Reference/list data** (`GET /api/*` except `/api/dashboard`): `StaleWhileRevalidate` — instantly shows the last-known answer, refreshes in the background. This is the "see the last things without a connection" requirement.
- **Dashboard** (`GET /api/dashboard`): `NetworkFirst` with a 4s timeout — prefers live data (it's the "what's happening right now" view) but falls back to the last cached snapshot when offline/slow.
...
- `src/components/PwaStatus.tsx` registers the service worker (manually, via `virtual:pwa-register`, `registerType: 'prompt'`) and owns the two pieces of UI a PWA needs: a persistent offline banner (`navigator.onLine` + `online`/`offline` listeners) and an "update available → reload" toast when a new service worker is waiting. Mount it once near the app root (already done in `App.tsx`) — don't duplicate the registration elsewhere.
