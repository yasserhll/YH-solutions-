...
- **Visibility**: a responsable can only create `type=expense` (auto-scoped to their own site), can never create an `entry` or `transfer`, and never sees the master `running_balance`/`current_balance`/`initial_balance` anywhere. They *do* see `transfer` rows crediting their own site (so their limit's history has a visible cause) and their own site's `site_running_balance`/`site_balance` — that's their own spending limit, they're allowed to see it. A SuperAdmin sees everything: the real master balance and every site's remaining limit (`summary.sites`) — there is deliberately no "grand total" KPI combining the two, since a site's limit isn't real money sitting anywhere, adding it to the master balance would double-count. Enforced server-side in `CashTransactionController` (`whereIn('type', ['expense','transfer'])` for a responsable's reads, `makeHidden('running_balance')` only — never `site_running_balance` — for a responsable), `CashAccountController@show` (role-shapes its response: SuperAdmin gets `{..., summary: {..., sites}}`, a responsable gets only `{site_id, site_balance}`), `DashboardController`, and `ReportController@cash`/`exportCash`. `routes/api.php`: `GET /cash-account` is available to any authenticated user (shaped by role in the controller); `PUT /cash-account` and `cash-transactions.{update,destroy}` stay SuperAdmin-only. The frontend (`CashPage.tsx`) mirrors this in what it renders (its table shows one "Reste global" column for a SuperAdmin — the real master balance — and one "Solde site" column for a responsable — their own limit; never both on the same table), but the backend restrictions are what actually matter.

### Cash ledger logic (`App\Services\CashLedgerService`)

`recalculate()` walks every transaction on the account in `(date, id)` order and maintains **two** independent running totals in the same pass, both via `bcadd`/`bcsub` on strings (never float): `running_balance` (the real master balance), moved only by `entry` (+) and `expense` (−); and `site_running_balance` (a site's remaining limit, per `site_id`), moved only by `transfer` (+, raises the destination site's limit) and `expense` (−, consumes the declaring site's limit) — null for `entry` rows. **`expense` debits BOTH at once** — it's the only type that represents real money leaving the company, so it has to move the true balance *and* eat into the site's allowance simultaneously; `transfer` never appears in the master ledger at all. Because a transaction can be edited or backdated, every write (`create`/`update`/`delete`) still recalculates the account's *entire* history, not just from the changed record forward.
