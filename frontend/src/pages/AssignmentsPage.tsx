...
export default function AssignmentsPage() {
  ...
  const columns: Column<Assignment>[] = [
    ...
    {
      header: 'Statut',
      // "sorti" means the EMPLOYEE left the company (see EmployeeStatus) —
      // a closed assignment row just means that period of it ended (a site
      // transfer, a department/position change...), which is a completely
      // different thing and must never share that badge, or a transferred-
      // but-still-active employee reads as if they'd quit.
      accessor: (a) => (a.is_current ? <StatusBadge status="actif" /> : <StatusBadge status="termine" />),
    },
    {
      header: 'Actions',
      accessor: (a) => (
        <div className="flex items-center gap-1">
          {a.is_current && (
            <button
              className="rounded-md p-1.5 text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-500/10"
              title="Affecter à un autre site"
  ...
  return (
    <div>
      ...
      {transferring && <TransferSiteModal assignment={transferring} onClose={() => setTransferring(null)} />}
...
function TransferSiteModal({ assignment, onClose }: { assignment: Assignment; onClose: () => void }) {
  const queryClient = useQueryClient();
  const { data: sites } = useSites();
  const [siteId, setSiteId] = useState('');
  const destinationSites = (sites ?? []).filter((s) => s.id !== assignment.site_id);

  const mutation = useMutation({
    mutationFn: () => api.post(`/employees/${assignment.employee_id}/transfer-site`, { site_id: siteId }),
    onSuccess: () => {
      toast.success('Employé affecté au nouveau site.');
      queryClient.invalidateQueries({ queryKey: ['assignments'] });
      queryClient.invalidateQueries({ queryKey: ['employees'] });
      onClose();
    },
  ...
  return (
    <Modal open onClose={onClose} title="Affecter à un autre site" size="sm">
      <form
        onSubmit={(e) => {
          e.preventDefault();
          mutation.mutate();
      ...
      >
        <div className="rounded-lg bg-slate-50 px-3 py-2 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300">
          <span className="font-medium text-slate-800 dark:text-slate-100">{assignment.employee?.full_name}</span> — actuellement à{' '}
          {assignment.site?.name}
        </div>
        <SelectField label="Nouveau site" required value={siteId} onChange={(e) => setSiteId(e.target.value)}>
        ...
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
          Tout l'historique de cet employé (pointage, congés, sanctions, heures sup., mouvements...) restera consultable — le responsable
          du nouveau site pourra voir l'historique complet, y compris celui d'avant ce transfert.
        </div>
        <div className="flex justify-end gap-2 pt-2">
          <Button type="button" variant="secondary" onClick={onClose}>
            Annuler
          </Button>
          <Button type="submit" disabled={!siteId || mutation.isPending}>
